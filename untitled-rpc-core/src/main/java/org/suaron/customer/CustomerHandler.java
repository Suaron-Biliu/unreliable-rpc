...
import static org.suaron.cahce.CustomerCache.RESP_MAP;

@Data
public class CustomerHandler  extends ChannelHandlerAdapter  {

    @Override
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception{
        //客户端和服务端之间的数据都是以RpcProtocol对象作为基本协议进行的交互
        UnreliableProtocol rpcProtocol = (UnreliableProtocol) msg;
        //这里是传输参数更为详细的RpcInvocation对象字节数组。
        byte[] reqContent = rpcProtocol.getContent();
        String json = new String(reqContent,0,reqContent.length);
        UnreliableInvocation rpcInvocation = JSON.parseObject(json, UnreliableInvocation.class);
